// ==UserScript==
// @name         Bili.Dynamic.AutoDel.Debug
// @namespace    https://github.com/
// @version      2026.09.03-debug
...
(function() {
    ...
    const LOG_PREFIX = '[Bili.Dynamic.AutoDel]';
    ...
    async function processDynamic(data, mode, input, lottery_api) {
        ...
        if (mode == 'auto') {
            ...
            let should_delete = lottery_info != undefined
                && ((lottery_info != null
            ...
            if (should_delete) {
                debugLog('自动判断：满足删除条件。', {
                    dynamic_id: data.id_str,
                    orig_id: orig_id_str,
                    reason: lottery_info != null && lottery_info.status == '2'
                        ? '抽奖状态为已开奖'
                        : lottery_info != null && lottery_info.end_timestamp != null
                            ? '已超过明确开奖时间'
                            : '已超过原动态发布时间加120天'
                });
                if (await deleteDynamic(data)) {
    ...
    function isLotteryDynamic(data) {
        ...
        return source_json.indexOf('RICH_TEXT_NODE_TYPE_LOTTERY') != -1
            || /抽奖|开奖|中奖|奖品/.test(source_json);
    }
